...
function currentGeminiModel(value, fallback) {
  var configured = String(value || '').trim();
  // Retired model IDs return HTTP 404. Do not let an old Netlify variable
  // disable Gemini measurements after the code has been upgraded.
  if (!configured || /^gemini-(?:1\.5|2\.0)(?:-|$)/i.test(configured)) return fallback;
  return configured;
}

const GEMINI_MODEL = currentGeminiModel(process.env.GEMINI_MODEL, 'gemini-3.5-flash');
