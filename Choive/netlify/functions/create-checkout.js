...
exports.handler = async function(event) {
  ...
  try {
    ...
    console.error('create-checkout: diagnostic lookup failed:', lookupError.message);
    return {
      statusCode: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  ...
  try {
    var res = await fetch('https://api.stripe.com/v1/checkout/sessions', {
      ...
      body: new URLSearchParams({
        ...
        'line_items[0][price_data][product_data][name]':    'CHOIVE Full Analysis',
        'line_items[0][price_data][product_data][description]': 'Unlock your full CHOIVE diagnostic — competitor intelligence, priority actions, pillar breakdown, and ready-to-use assets.',
